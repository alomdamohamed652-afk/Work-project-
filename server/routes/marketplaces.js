...
router.get('/:idOrSlug',requireAuth,requireRole('customer'),async(req,res,next)=>{try{
  ...
  const merchants=await pool.query(`SELECT u.id,COALESCE(rp.display_name,u.full_name) name,rp.logo_url,rp.is_open,rp.preparation_minutes,COALESCE((SELECT json_agg(DISTINCT mi.marketplace_category_id) FROM menu_items mi WHERE mi.restaurant_id=u.id AND mi.is_available=true AND mi.marketplace_category_id IS NOT NULL),'[]'::json) marketplace_category_ids,COALESCE((SELECT json_agg(DISTINCT mi.marketplace_subcategory_id) FROM menu_items mi WHERE mi.restaurant_id=u.id AND mi.is_available=true AND mi.marketplace_subcategory_id IS NOT NULL),'[]'::json) marketplace_subcategory_ids FROM restaurant_profiles rp JOIN users u ON u.id=rp.restaurant_id WHERE rp.marketplace_id=$1 AND u.status='active' AND u.role='restaurant' ORDER BY rp.is_featured DESC,name LIMIT 100`,[marketplace.id]);
