...
router.get("/restaurant/:restaurantId", requireAuth, requireRole("customer"), async (req, res, next) => {
  try {
    const { rows } = await pool.query(
      `SELECT r.id, COALESCE(rp.display_name,r.full_name) AS restaurant_name,
              COALESCE(rp.area,r.area) AS area, COALESCE(rp.address,r.address) AS address,
              rp.logo_url, rp.cover_url,
