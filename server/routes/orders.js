  ...
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO orders
       ...
       RETURNING *`,
      [req.user.id, b.restaurantId || null, hasCoordinates ? Number(b.deliveryLatitude) : null, hasCoordinates ? Number(b.deliveryLongitude) : null, deliveryAddress || null, total]
    );
...
router.get("/mine", requireAuth, requireRole("customer"), async (req, res, next) => {
  try {
    const { rows } = await pool.query(`SELECT o.*, u.full_name AS driver_name, u.phone AS driver_phone,
              l.latitude AS driver_latitude, l.longitude AS driver_longitude, l.accuracy AS driver_accuracy,
              l.heading AS driver_heading, l.speed AS driver_speed, l.updated_at AS driver_location_updated_at
       FROM orders o LEFT JOIN users u ON u.id=o.driver_id LEFT JOIN user_locations l ON l.user_id=o.driver_id
...
router.get("/driver/available", requireAuth, requireRole("driver"), async (_req, res, next) => {
  try {
    const { rows } = await pool.query(`SELECT o.id, o.status, o.restaurant_id, o.delivery_latitude, o.delivery_longitude,
              o.delivery_address, o.total_amount, o.created_at, o.updated_at, c.full_name AS customer_name, c.phone AS customer_phone
...
router.patch("/:id/claim", requireAuth, requireRole("driver"), async (req, res, next) => {
  try {
    const { rows } = await pool.query(`UPDATE orders SET driver_id=$1, status='assigned', updated_at=now()
       WHERE id=$2 AND driver_id IS NULL AND status IN ('confirmed','preparing','ready') RETURNING *`, [req.user.id, req.params.id]);
...
router.get("/driver/mine", requireAuth, requireRole("driver"), async (req, res, next) => {
  try {
    const { rows } = await pool.query(`SELECT o.id, o.status, o.delivery_latitude, o.delivery_longitude,
              o.delivery_address, o.total_amount, o.created_at, o.updated_at, c.full_name AS customer_name,
              c.phone AS customer_phone, c.secondary_phone AS customer_secondary_phone
       FROM orders o JOIN users c ON c.id=o.customer_id
       WHERE o.driver_id=$1 AND o.status IN ('assigned','picked_up','on_the_way') ORDER BY o.updated_at DESC`, [req.user.id]);
    res.json({ orders: rows });
  } catch (error) { next(error); }
});
...
router.get("/admin", requireAuth, requireRole("admin", "staff"), async (_req, res, next) => {
  try {
    const { rows } = await pool.query(`SELECT o.*, c.full_name AS customer_name, c.phone AS customer_phone,
              d.full_name AS driver_name, d.phone AS driver_phone FROM orders o JOIN users c ON c.id=o.customer_id
       LEFT JOIN users d ON d.id=o.driver_id ORDER BY o.updated_at DESC LIMIT 200`);
    res.json({ orders: rows });
  } catch (error) { next(error); }
});
...
router.patch("/:id/assign", requireAuth, requireRole("admin", "staff"), async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(`UPDATE orders SET driver_id=$1, status='assigned', updated_at=now()
       WHERE id=$2 AND status NOT IN ('delivered','cancelled') RETURNING *`, [driverId, req.params.id]);
...
router.patch("/:id/status", requireAuth, requireRole("driver"), async (req, res, next) => {
  try {
    ...
    if (!activeDeliveryStatuses.includes(status) && status !== "delivered") return res.status(400).json({ error: "حالة الطلب غير صحيحة" });
    const { rows } = await pool.query(`UPDATE orders SET status=$1, updated_at=now()
       WHERE id=$2 AND driver_id=$3 AND status NOT IN ('delivered','cancelled') RETURNING *`, [status, req.params.id, req.user.id]);
