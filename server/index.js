...
// ✅ FIXED: Router Order
// /api/orders is mounted in this order:
// 1. orderWorkflowRoutes - workflow endpoints (high priority)
// 2. orderRoutes - main order endpoints (requireRole("customer", "restaurant", "admin", "staff"))
// 3. orderProofRoutes - proof endpoints
// 4. orderItemAdjustmentRoutes - item adjustments
// 5. driverDispatchRoutes - driver /available and /:id/claim (requireRole("driver"))
// 
// BEFORE (BUG): driverDispatchRoutes was mounted BEFORE orderRoutes
// This caused requireRole("driver") to reject customer requests to /api/orders/bulk and /api/orders/quote
// NOW: driverDispatchRoutes is AFTER orderRoutes, so customer routes match first
...
app.use('/api/orders', driverDispatchRoutes); // ✅ MOVED to end (was before orderRoutes)
...
app.use((err, _req, res, _next) => {
  ...
  const msg =
    err.message === 'DELIVERY_PROOF_REQUIRED'
...
async function processPaymentAdjustmentNotifications() {
  try {
    const { rows } = await pool.query(
      `SELECT id,customer_id,order_id,amount FROM payment_adjustments WHERE status='pending' AND customer_notified_at IS NULL ORDER BY created_at ASC LIMIT 100`
    );
    ...
    for (const x of rows) {
      try {
        ...
        await pool.query(
          `UPDATE payment_adjustments SET customer_notified_at=now(),updated_at=now() WHERE id=$1 AND customer_notified_at IS NULL`,
          [x.id]
        );
      } catch (e) {
...
async function start() {
  ...
  setInterval(
    async () => {
      try {
        const { rows } = await pool.query(
          `SELECT sc.id,sc.customer_id FROM support_conversations sc WHERE sc.status IN ('open','pending') AND sc.needs_reply=true AND sc.followup_sent_at IS NULL AND sc.first_customer_message_at IS NOT NULL AND sc.last_staff_message_at IS NULL AND sc.first_customer_message_at<=now()-interval '2 minutes'`
        );
        for (const x of rows) {
          await notifyUserSafe(x.customer_id);
          await pool.query('UPDATE support_conversations SET followup_sent_at=now(),updated_at=now() WHERE id=$1', [
            x.id,
          ]);
        }
      } catch (e) {
