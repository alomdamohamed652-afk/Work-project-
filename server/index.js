...
const migrateOrderStatusCleanup = require('./migrate_order_status_cleanup');
const migrateAuditHardening = require('./migrate_audit_hardening');
...
async function cleanupLocationHistory() { try { await withAdvisoryLock(81003, async (client) => { const { rowCount } = await client.query(`DELETE FROM location_history WHERE created_at < now() - interval '30 days'`); if (rowCount) console.log(`Location history cleanup removed ${rowCount} expired records`); }); } catch (e) { console.error('Location history cleanup failed', e.message); } }
