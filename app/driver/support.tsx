...
export default function DriverSupport(){
 ...
 if(selected)return <SafeAreaView style={s.safe}><KeyboardAvoidingView style={{flex:1}} behavior={Platform.OS==='ios'?'padding':'height'}>
  ...
  <ScrollView style={{flex:1}} contentContainerStyle={s.messages} keyboardShouldPersistTaps="handled">{messages.map(m=><View key={m.id} style={[s.bubble,m.sender_role==='driver'?s.mine:s.theirs]}><Text style={[s.bubbleText,m.sender_role==='driver'&&s.mineText]}>{m.body}</Text><View style={s.messageMeta}><Text style={[s.messageMetaText,m.sender_role==='driver'&&s.messageMetaMine]}>{m.sender_name|| (m.sender_role==='driver'?'أنت':'فريق الدعم')}</Text><Text style={[s.messageMetaText,m.sender_role==='driver'&&s.messageMetaMine]}>{msgTime(m.created_at)}</Text></View></View>)}</ScrollView>
