...
export default function Catalog(){
  ...
  const [badgeName,setBadgeName]=useState(''),[badgeIcon,setBadgeIcon]=useState('🏷️'),[badgeDescription,setBadgeDescription]=useState(''),[editingBadge,setEditingBadge]=useState<any>(null);
  ...
  const resetBanner=()=>{setTitle('');setSubtitle('');setImage('');setAction('');setEditingBanner(null)};
  const addBanner=()=>run(async()=>{if(!title.trim())throw Error('اكتب عنوان البانر');if(editingBanner)await request('/api/admin/catalog/banners/'+editingBanner.id,'PATCH',{title,subtitle,imageUrl:image,actionLabel:action});else await request('/api/admin/catalog/banners','POST',{title,subtitle,imageUrl:image,actionLabel:action});resetBanner();await load()});
  const editBanner=(x:any)=>{setEditingBanner(x);setTitle(x.title||'');setSubtitle(x.subtitle||'');setImage(x.image_url||x.imageUrl||'');setAction(x.action_label||x.actionLabel||'')};
  const toggleBanner=(x:any)=>run(async()=>{await request('/api/admin/catalog/banners/'+x.id,'PATCH',{isActive:!x.is_active});await load()});
  const deleteBanner=(x:any)=>Alert.alert('حذف البانر','سيتم حذفه من واجهة العميل.',[{text:'إلغاء',style:'cancel'},{text:'حذف',style:'destructive',onPress:()=>run(async()=>{await request('/api/admin/catalog/banners/'+x.id,'DELETE');await load()})}]);
  ...
  const addBadge=()=>run(async()=>{if(!badgeName.trim())throw Error('اكتب اسم البادج');if(editingBadge)await request('/api/admin/catalog/badges/'+editingBadge.id,'PATCH',{name:badgeName,icon:badgeIcon,description:badgeDescription});else await request('/api/admin/catalog/badges','POST',{name:badgeName,icon:badgeIcon,description:badgeDescription});setBadgeName('');setBadgeIcon('🏷️');setBadgeDescription('');setEditingBadge(null);await load()});
  const editBadge=(x:any)=>{setEditingBadge(x);setBadgeName(x.name||'');setBadgeIcon(x.icon||'🏷️');setBadgeDescription(x.description||'')};
  ...
  return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
    ...
    {tab==='banners'?<><Hint text="البانر يحتاج عنوانًا وصورة ورابطًا اختياريًا للوجهة."/><Field value={title} set={setTitle} ph="عنوان البانر"/><Field value={subtitle} set={setSubtitle} ph="وصف مختصر"/><Field value={image} set={setImage} ph="رابط صورة البانر (URL)"/><Field value={action} set={setAction} ph="اسم زر البانر"/><Pressable disabled={busy} onPress={addBanner} style={s.primary}><Text style={s.btn}>{editingBanner?'حفظ تعديل البانر':'+ إضافة بانر'}</Text></Pressable>{editingBanner?<Pressable onPress={resetBanner} style={s.cancel}><Text style={s.cancelText}>إلغاء التعديل</Text></Pressable>:null}{banners.map(x=><View key={x.id} style={[s.card,!x.is_active&&s.off]}><Text style={s.name}>{x.title}</Text><Text style={s.muted}>{x.subtitle||'بدون وصف'} • {x.is_active?'ظاهر':'مخفي'}</Text><View style={s.actions}><Pressable onPress={()=>editBanner(x)} style={s.action}><Text style={s.actionText}>تعديل</Text></Pressable><Pressable onPress={()=>toggleBanner(x)} style={s.action}><Text style={x.is_active?s.warn:s.good}>{x.is_active?'إخفاء':'إظهار'}</Text></Pressable><Pressable onPress={()=>deleteBanner(x)} style={s.action}><Text style={s.bad}>حذف</Text></Pressable></View></View>)}</>
    ...
    :<><Hint text="1) أنشئ البادج هنا. 2) افتح المطعم من إدارة المطاعم. 3) اختر التصنيفات والبادجات لإسناده للمطعم."/><Field value={badgeName} set={setBadgeName} ph="اسم البادج — مثال: الأكثر طلبًا"/><Field value={badgeIcon} set={setBadgeIcon} ph="الأيقونة — مثال: 🔥"/><Field value={badgeDescription} set={setBadgeDescription} ph="وصف البادج (اختياري)"/><Pressable disabled={busy} onPress={addBadge} style={s.primary}><Text style={s.btn}>{editingBadge?'حفظ تعديل البادج':'+ إنشاء بادج'}</Text></Pressable>{editingBadge?<Pressable onPress={()=>{setEditingBadge(null);setBadgeName('');setBadgeIcon('🏷️');setBadgeDescription('')}} style={s.cancel}><Text style={s.cancelText}>إلغاء التعديل</Text></Pressable>:null}{badges.map(x=><View key={x.id} style={[s.card,!x.is_active&&s.off]}><Text style={s.name}>{x.icon||'🏷️'} {x.name}</Text><Text style={s.muted}>{x.description||'بدون وصف'} • {x.is_active?'نشط':'مخفي'}</Text><View style={s.actions}><Pressable onPress={()=>editBadge(x)} style={s.action}><Text style={s.actionText}>تعديل الاسم</Text></Pressable><Pressable onPress={()=>toggleBadge(x)} style={s.action}><Text style={x.is_active?s.warn:s.good}>{x.is_active?'إخفاء':'إظهار'}</Text></Pressable><Pressable onPress={()=>deleteBadge(x)} style={s.action}><Text style={s.bad}>حذف</Text></Pressable></View></View>)}</>}
