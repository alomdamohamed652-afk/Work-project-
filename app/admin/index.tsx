  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!response.ok || data.user?.role !== "admin") { router.replace("/home"); return; }
      } catch { router.replace("/auth"); return; }
      finally { setChecking(false); }
    })();
  }, []);

  if (checking) return <SafeAreaView style={styles.safe}><View style={styles.loading}><ActivityIndicator size="large" color={theme.primary} /><Text style={styles.loadingText}>جاري التحقق من صلاحيات الإدارة...</Text></View></SafeAreaView>;
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView style={styles.page} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.stats}>
          <Stat label="طلبات اليوم" value="0" /><Stat label="مندوبون متصلون" value="0" /><Stat label="تحتاج تدخل" value="0" />
        </View>
        <Text style={styles.section}>الوصول السريع</Text>
        {cards.map(([icon, title, description]) => {
          const enabled = title === "المستخدمون" || title === "الإعدادات";
          return <Pressable key={title} disabled={!enabled} onPress={() => { if (title === "المستخدمون") router.push("/admin/users"); if (title === "الإعدادات") router.push("/admin/settings"); }} style={({ pressed }) => [styles.card, !enabled && styles.disabledCard, pressed && styles.pressed]}>
